...
@WebService(endpointInterface = "dsd.Event_Interface")
@SOAPBinding(style = SOAPBinding.Style.RPC)

public class Otw_server implements Event_Interface
{
	...
	void receive() {
		...
		try {
			...
			while(true) {
				...
				else if("AlreadyPresent".equalsIgnoreCase(purpose)) {
					String customerID = new String(receivepacket[1]);
					String eventID = new String(receivepacket[2]);
					String eventType = new String(receivepacket[3]);						
					System.out.println("going to alreadyPresent event");
					boolean msg1=alreadyPresent(customerID.trim(),eventID.trim(),eventType.trim());
					if(msg1=false) {
					msg="false";
					}
					else
...
public String bookEvent(String customerID, String eventID, String eventType)
{
  ...
  if(eventID.substring(0,3).equalsIgnoreCase("OTW")) 
  {
	...
	public String swapEvent(String customerID, String newEventID, String newEventType, String oldEventID,
			...
			msg2=cancelEvent(customerID,oldEventID,oldEventType);
			System.out.println(msg2);
			if(msg2.equalsIgnoreCase(oldEventID+" Event has been removed.")) {
				msg=bookEvent(customerID,newEventID,newEventType);
				if(msg.equalsIgnoreCase("Booking for "+newEventID+" is succesful for the " +customerID)) {
					msg="Events are Swaped Succesfully";
					return msg;
				}
				else {
					String msg1=bookEvent(customerID,oldEventID,oldEventType);
					msg="Swaping not succesfull because event can't be booked";
					}
				}
			else {
				msg="Event not booked, so can't cancel";
			}
			return msg;
	...
	public static void main(String[] args) throws IOException 
	{
		...
		Endpoint.publish("http://localhost:9020/otw", ms);   

		ms.inimain();
	}

	protected Otw_server() throws IOException {
		super();
